    ...
    $class_counter  = 1;
        ...
        if( count($all_images) > 0 ):
            foreach( $all_images as $single_image ){
                ...
                $property_featured_image_url = wpestate_get_property_image_src($single_image, 'property_full_map');
                ...
                <div class="item  prop_animation_class_'.esc_attr($class_counter).' '.esc_attr($active).' " data-id="'.esc_attr($counter).'" style=" height:'.esc_attr($theme_slider_height).'px;" >
                    ...
                    <div class="image_div">
                        <img src="'.esc_url($property_featured_image_url).'" alt="'.esc_html__('user image','wpresidence').'">
                    </div>
